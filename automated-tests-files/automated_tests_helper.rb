...
# Original Prototype code -- left here as a reference
...
# $('test_script_options').insert({bottom: "#{ escape_javascript test_script_options }".replace(/(attributes_\\d+|\\[\\d+\\])/g, new_test_script_id) });
  ...
  # Checks if a test script or test support file 
  # can be downloaded. Check three things:
  #
  #   1. file is in DB
  #   2. file is in the directory it's supposed to be
  #   3. file exists and is readable
  #
  # If all three are true, then the file can be downloaded, 
  #   and the function returns a string indicating the file type
  # (binary or text)
  # Otherwise, returns false.
  #
  #def safe_to_download?(assn_id, filename)
  #  testscript = TestScript.find_by_assignment_id_and_script_name(assn_id, filename)
  #  if testscript
  #    filename = testscript.script_name
  #    assn_short_id = Assignment.find(assn_id).short_identifier
  ...
  #    filename = File.join(should_be_in, filename)

  #    if should_be_in == File.expand_path(File.dirname(filename)) and File.readable?(filename)
  #      # Everything looks OK. Send the file over to the client.
  #      file_contents = IO.read(filename)
  #      return SubmissionFile.is_binary?(file_contents) ? 'application/octet-stream' : 'text/plain'
  #    end
  #  end
  #  false
  #end
